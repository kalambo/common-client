...
exports.default = (function (fileServer) {
    ...
    return mishmash_1.default
    ...
    }, mishmash_1.default.merge('value', 'onChange', 'index', function (value, onChange, index) { return ({
        value: (value && value[index]) || null,
        onChange: function (indexValue) {
            var newValue = __spread(__spread(new Array(index)).map(function (_, i) { return common_1.noUndef(value && value[i]); }), [
                indexValue
            ], (value || []).slice(index + 1).map(common_1.noUndef));
            onChange(newValue.some(function (v) { return v !== null; }) ? newValue : null);
        },
    }); }))
        .doIf('view', mishmash_1.default
        .merge('style', function (style) {
        return ({
            style: (_a = style_transform_1.default(style)).filter.apply(_a, __spread(elmnt_1.css.groups.text)),
        });
        var _a;
    })
        .yield(function (_a) {
        ...
        .merge('relation', 'label', 'data', function (relation, label, data) { return ({
        options: data ? data[relation].map(function (d) { return d.id; }) : [],
        labels: data
            ? data[relation].map(function (d) {
                return Array.isArray(label)
                    ? label[1].apply(label, __spread(label[0].map(function (k) { return d[k]; }))) : d[label];
            })
            : [],
    }); }))
        .doIf('other', mishmash_1.default
        .merge('value', 'options', function (value, options) { return ({
        otherOpen: value !== null && !options.includes(value),
    }); })
        .merge('onChange', 'other', function (onChange, other, push) { return ({
