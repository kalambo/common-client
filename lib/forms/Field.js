"use strict";
var __assign = (this && this.__assign) || Object.assign || function(t) {
    for (var s, i = 1, n = arguments.length; i < n; i++) {
        s = arguments[i];
        for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
            t[p] = s[p];
    }
    return t;
};
var __rest = (this && this.__rest) || function (s, e) {
    var t = {};
    for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
        t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) if (e.indexOf(p[i]) < 0)
            t[p[i]] = s[p[i]];
    return t;
};
var __read = (this && this.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
        while ((n === void 0 || n-- > 0) && !(r = i.next()).done) ar.push(r.value);
    }
    catch (error) { e = { error: error }; }
    finally {
        try {
            if (r && !r.done && (m = i["return"])) m.call(i);
        }
        finally { if (e) throw e.error; }
    }
    return ar;
};
var __spread = (this && this.__spread) || function () {
    for (var ar = [], i = 0; i < arguments.length; i++) ar = ar.concat(__read(arguments[i]));
    return ar;
};
Object.defineProperty(exports, "__esModule", { value: true });
var React = require("react");
var elmnt_1 = require("elmnt");
var mishmash_1 = require("mishmash");
var common_1 = require("common");
var style_transform_1 = require("style-transform");
var getData_1 = require("../generic/getData");
var Hover = mishmash_1.default()
    .enhance(mishmash_1.watchHover)
    .toComp();
exports.default = (function (fileServer) {
    var FileButton = mishmash_1.default()
        .branch(function (_a) {
        var value = _a.value;
        return !value;
    }, mishmash_1.default().render())
        .style([['mergeKeys', 'button'], ['merge', { width: '100%' }]])(function (_a) {
        var value = _a.value, style = _a.style;
        return (React.createElement("a", { href: fileServer + "/storage/file/" + value.split(':')[0], target: "_blank" },
            React.createElement(Hover, null, function (_a) {
                var hover = _a.isHovered, hoverProps = _a.hoverProps;
                return (React.createElement(elmnt_1.Txt, __assign({}, hoverProps, { style: style_transform_1.default(__assign({}, style, { fontSize: 15, padding: 8 }), [
                        ['mergeKeys', { hover: hover }],
                    ]) }), "View file"));
            })));
    });
    return mishmash_1.default()
        .branch(function (_a) {
        var index = _a.index;
        return index !== undefined;
    }, mishmash_1.default()
        .enhance(function (_a) {
        var methods = _a.methods;
        return function (props) { return (__assign({}, props, methods({
            onChange: function (indexValue) {
                var newValue = __spread(__spread(new Array(props.index)).map(function (_, i) {
                    return common_1.noUndef(props.value && props.value[i]);
                }), [
                    indexValue
                ], (props.value || []).slice(props.index + 1).map(common_1.noUndef));
                props.onChange(newValue.some(function (v) { return v !== null; }) ? newValue : null);
            },
        }))); };
    })
        .map(function (_a) {
        var value = _a.value, props = __rest(_a, ["value"]);
        return (__assign({}, props, { value: (value && value[props.index]) || null }));
    }))
        .branch(function (_a) {
        var view = _a.view;
        return view;
    }, mishmash_1.default()
        .style([__spread(['filter'], elmnt_1.css.groups.text)])
        .render(function (_a) {
        var type = _a.type, value = _a.value, style = _a.style;
        return (React.createElement(elmnt_1.Txt, { style: style }, common_1.getValueString(value, type)));
    }))
        .branch(function (_a) {
        var type = _a.type, admin = _a.admin;
        return type === 'file' && admin;
    }, mishmash_1.default().render(function (_a) {
        var value = _a.value, style = _a.style, next = _a.next;
        return (React.createElement("div", { style: { width: '100%' } },
            React.createElement(elmnt_1.Div, { style: { spacing: 40, layout: 'bar', width: '100%' } },
                React.createElement("div", { style: { width: 150 } },
                    React.createElement(FileButton, { value: value, style: style })),
                next())));
    }))
        .branch(function (_a) {
        var type = _a.type, options = _a.options, admin = _a.admin;
        return admin && !type.endsWith('list') && Array.isArray(options);
    }, mishmash_1.default().map(function (_a) {
        var options = _a.options, labels = _a.labels, props = __rest(_a, ["options", "labels"]);
        return (__assign({}, props, { options: options && (!options.includes(null) ? __spread(options, [null]) : options), labels: labels &&
                (!options.includes(null) ? __spread(labels, ['-- None --']) : labels) }));
    }))
        .branch(function (_a) {
        var relation = _a.relation;
        return relation;
    }, mishmash_1.default()
        .merge(getData_1.default(function (_a) {
        var relation = _a.relation, filter = _a.filter, sort = _a.sort, label = _a.label;
        return ({
            name: relation,
            filter: filter,
            sort: sort,
            fields: __spread(['id'], (Array.isArray(label) ? label[0] : [label])),
        });
    }))
        .map(function (props) { return (__assign({}, props, { options: props.data ? props.data[props.relation].map(function (d) { return d.id; }) : [], labels: props.data
            ? props.data[props.relation].map(function (d) {
                return Array.isArray(props.label)
                    ? (_a = props.label)[1].apply(_a, __spread(props.label[0].map(function (k) { return d[k]; }))) : d[props.label];
                var _a;
            })
            : [] })); }))
        .branch(function (_a) {
        var other = _a.other;
        return other;
    }, mishmash_1.default()
        .enhance(function (_a) {
        var firstProps = _a.firstProps, setState = _a.setState, methods = _a.methods;
        setState({
            otherOpen: firstProps.value !== null &&
                !firstProps.options.includes(firstProps.value),
        });
        return function (props, state) { return (__assign({}, props, state, methods({
            onBaseChange: function (value) {
                if (value === props.other) {
                    setState({ otherOpen: true });
                    props.onChange(null);
                }
                else {
                    setState({ otherOpen: false });
                    props.onChange(value);
                }
            },
        }))); };
    })
        .render(function (_a) {
        var value = _a.value, options = _a.options, other = _a.other, otherOpen = _a.otherOpen, onBaseChange = _a.onBaseChange, props = __rest(_a, ["value", "options", "other", "otherOpen", "onBaseChange"]);
        return (React.createElement(elmnt_1.Div, { style: { spacing: 10 } },
            React.createElement(elmnt_1.Input, __assign({}, props, { value: otherOpen ? other : value, onChange: onBaseChange, options: __spread(options, [other]) })),
            otherOpen && (React.createElement(elmnt_1.Input, __assign({}, props, { value: value, placeholder: "Please specify" })))));
    }))(elmnt_1.Input);
});
