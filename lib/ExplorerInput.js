...
exports.default = mishmash_1.default()
    ...
    firstProps.context.store.watch('editing', function (editing) {
        if (editing === void 0) { editing = {}; }
        return setState({ editing: editing });
    }, onProps);
    var onChange = function (value) {
        return firstProps.context.store.update('editing', function (v) { return (__assign({}, v, { value: value })); });
    };
    var onTextChange = function (text) {
        setState({ text: text });
        setTimeout(function () { return firstProps.context.updateWidths(); });
    };
    var lastValue = firstProps.context.store.get('editing').value;
