...
var csvHeader = function (config, types, _a, type) {
    ...
    var blocks = fields.map(function (f) {
        ...
        var fieldName = type
            ...
            : types[f.name].name;
        var newType = type ? utils_1.root.rgo.schema[type][f.name].type : f.name;
                        ...
                        f === '' || values === undefined
                            ...
                            : f.startsWith('#')
                                ...
                                : config.printValue(value, f === 'id'
                                    ? { scalar: 'string' }
                                    : utils_1.root.rgo.schema[type][f]),
                    ],
                ];
            }
            return csvData(config, __spread(['#'], (f.fields.length === 0 ? [''] : f.fields)), (values || {})[f.alias || f.name], type ? utils_1.root.rgo.schema[type][f.name].type : f.name, f.start || 0);
        });
        var height = Math.max.apply(Math, __spread(blocks.map(function (rows) { return rows.length; })));
