...
})(mishmash_1.isolate(function (elem, firstProps, onProps) {
    var Input = firstProps.context.input;
    var startEditing = function (key, value) {
        firstProps.context.store.set('editing', { key: key, value: value });
        firstProps.context.store.update('initial', function (initial) {
            if (initial === void 0) { initial = {}; }
            return (__assign({}, initial, (initial[key] === undefined ? (_a = {}, _a[key] = value, _a) : {})));
            var _a;
        });
    };
    var stopEditing = function (invalid) {
        var _a = firstProps.context.store.get('editing'), key = _a.key, value = _a.value;
        firstProps.context.store.set('editing', {});
        firstProps.context.store.update('initial', function (_a) {
            var _b = key, v = _a[_b], initial = __rest(_a, [typeof _b === "symbol" ? _b : _b + ""]);
            ...
            return __assign({}, initial, (_c = {}, _c[key] = v, _c));
            var _c;
        });
    };
    var inputRef = null;
    var unlisten = firstProps.context.store.listen('editing', function (editing) {
        ...
        if (editing.key) {
            ...
            for (var i = 0; i < elems.length; i++) {
                if (elems[i] !== inputRef) {
                    elems[i].textContent = firstProps.context.config.printValue(editing.value, common_1.root.rgo.schema[splitKey[0]][splitKey[2]].scalar);
                }
            }
        }
    });
    onProps(function (props) {
        if (props) {
            ...
            var allCells = cells
                ...
                .append('td')
                .merge(cells)
                .datum(function (d) { return (__assign({}, d, { style: inputRef !== _this && Object.keys(props.initial).includes(d.key)
                    ? 'changed'
                    : d.empty
                        ? 'empty'
            ...
            allCells
                ...
                if (inputRef === this) {
                    ...
                    if (editing_1.key === key) {
                        this.textContent = props.context.config.printValue(editing_1.value, common_1.root.rgo.schema[type][field].scalar);
                    }
                    else {
