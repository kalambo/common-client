...
exports.default = mishmash_1.default
    ...
    var base = (_a = style_transform_1.default(style)
        .mergeKeys('data')
    ...
    })).filter.apply(_a, __spread(elmnt_1.css.groups.text, ['padding', 'border', 'background', 'maxWidth'])).merge({
        position: 'relative',
        verticalAlign: 'top',
        whiteSpace: 'pre-wrap',
    ...
    return {
        style: {
            ...
            input: style_transform_1.default(style)
    ...
    .merge('context', 'query', 'data', function (context, query, data) { return ({
...
})(mishmash_1.isolate(function (elem, props$) {
    ...
    var unlisten = props$().context.store.listen('editing', function (editing) {
        ...
        if (editing.key) {
            ...
            for (var i = 0; i < elems.length; i++) {
                if (elems[i] !== inputRef) {
                    elems[i].textContent = props$().context.config.printValue(editing.value, common_1.root.rgo.schema[splitKey[0]][splitKey[2]]);
                }
            }
        }
    });
    props$('context', 'dataRows', 'style', 'unchanged', function (context, dataRows, style, unchanged) {
        ...
        allCells
            ...
            if (inputRef === this) {
                ...
                ReactDOM.unmountComponentAtNode(this);
                if (editing.key === key) {
                    this.textContent = context.config.printValue(editing.value, common_1.root.rgo.schema[type][field]);
                }
                else {
