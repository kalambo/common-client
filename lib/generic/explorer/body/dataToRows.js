...
var dataToRows = function (context, fields, data, type, start, initial, first) {
    ...
    return dataArray.reduce(function (result, values, i) {
        var dataBlocks = fields.map(function (f, j) {
                    ...
                    [
                        {
                            ...
                            text: f === '' || values === undefined
                                ...
                                : f.startsWith('#')
                                    ? "" + (start + i + 1)
                                    : context.config.printValue(value, f === 'id'
                                        ? 'string'
            ...
            ]), (values || {})[f.alias || f.name], type ? common_1.root.rgo.schema[type][f.name].type : f.name, f.start || 0, false, first && i === 0);
        });
        ...
        return __spread(result, dataBlocks.reduce(function (res, rows) {
            ...
            if (rows[0][0].span !== undefined && height > rows.length) {
                res[rows.length] = __spread((res[rows.length] || []), Array.from({ length: rows[0].length }).map(function (_, j) { return (__assign({}, rows[0][j], { text: undefined, span: height - rows.length, first: false, empty: true })); }));
            }
            return res;
        }, []));
    }, []);
