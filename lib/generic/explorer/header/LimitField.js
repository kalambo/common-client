...
exports.default = mishmash_1.default
    ...
    return {
        style: __assign({}, style, { input: input, div: (_a = style_transform_1.default(input)
                .scale({ margin: { padding: -1 } })).filter.apply(_a, __spread(['margin'], (active && !focused ? ['background'] : []))).merge({ position: 'relative', zIndex: focused ? 30 : 6 }), text: (_b = style_transform_1.default(input)).filter.apply(_b, __spread(elmnt_1.css.groups.text, ['padding'], (active && !focused ? [] : ['background']))).scale({ minWidth: { fontSize: 2 } })
                .merge({ display: 'inline-block', verticalAlign: 'top' }), arrow: style_transform_1.default(input)
                .mergeKeys('connect')
                .scale({
                fontSize: 0.9,
                paddingTop: 0,
