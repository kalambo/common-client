...
exports.default = refluent_1.default
    ...
    return __assign({}, style, { input: input, div: input
            .scale({ margin: { padding: -1 } })
            .filter('margin', 'background')
            .merge({ position: 'relative' }), bar: input
            ...
            .scale({ fontSize: 0.8 })
            .filter('color', 'fontSize', 'padding'), iconWidth: input.scale({
            width: { fontSize: 0.8, paddingLeft: 0.5, paddingRight: 0.5 },
        }), text: input
            .filter.apply(input, __spread(elmnt_1.css.groups.text, ['padding'])).scale({ paddingRight: 2 }) });
}))
